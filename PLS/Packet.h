...
void PacketAcceptQuest(ICharacter IPlayer, void* pPacket, int pPos)
{
	...
	IPlayer.SystemMessage(Int2String(IConfig::dungeon_vector.end() - IConfig::dungeon_vector.begin()), TEXTCOLOR_BLUE);


	auto it = std::find_if(IConfig::dungeon_vector.begin(), IConfig::dungeon_vector.end(), [index = ID](const auto &c) {return index == c.quest_id; });
	IPlayer.SystemMessage(Int2String(IConfig::dungeon_vector.size()), TEXTCOLOR_RED);
	IPlayer.SystemMessage(Int2String(ID), TEXTCOLOR_RED);
	if (it != IConfig::dungeon_vector.end())
	{
		IPlayer.SystemMessage(Int2String(IConfig::dungeon_vector.size()), TEXTCOLOR_RED);

		//for (auto& i : IConfig::dungeon_vector)
		//{
		//	IPlayer.SystemMessage(Int2String(i.quest_id), TEXTCOLOR_RED);
		//}



		IPlayer.SystemMessage("YIKES", TEXTCOLOR_CLASSMATE);
		IPlayer.SystemMessage(Int2String(it->quest_id), TEXTCOLOR_RED);
