...
int __fastcall NormalHit(void *Player, void *edx, int bType, int nTargetID)
{
	...
	if (IPlayer.IsValid() && nTargetID && IPlayer.GetType() == 0 && IPlayer.GetClass() == 3 && !CChar::IsGState((int)IPlayer.Offset, 512) && IPlayer.IsBuff(333) && DanceOfKillingON == true)
	{
		int Target = 0;
		...
		if (IPlayer.IsValid() && Target)
		{
			...
			if (IPlayer.IsValid() && ITarget.IsValid() && IPlayer.CheckHit(ITarget, 10) && IPlayer.GetMap() == ITarget.GetMap())
			{
				...
				if (IPlayer.IsValid() && pSkill)
				{
					...
					int nDmg = IPlayer.GetAttack()*(DoKDamageMultiPvE / 100);

					if (ITarget.GetType() == 0)
						nDmg = IPlayer.GetAttack()*(DoKDamageMultiPvP / 100);
					...
					*(DWORD*)((int)Player + 1448) = Check + GetTickCount();
					...
					if (CheckNormalDamage && *((DWORD*)IPlayer.GetOffset() + 70) & 2146435072 && ((*((DWORD *)IPlayer.GetOffset() + 70) & 2147483648) != 0 ? 10 : 5) > (int)CTools::Rate(0, 99))
					{
			...
			CSkill::ObjectRelease(ITarget.GetOffset(), (int)Target + 352);
			return 0;
		}
	}
