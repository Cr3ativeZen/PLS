...
void IConfig::LoadSkillFormulas()
{
	...
	if (filez != NULL)
	{
		...
		{
			int character = 0, skill_id = 0, str = 0, agi = 0, wis = 0, inte = 0, base_damage = 0, damage_per_grade = 0, enabled = 0, pvp_reduction = 0, damageC = 0, per_deathblow = 0, buff_id = 0, duration_per_grade = 0, duration = 0;
			...
			//Debuffs like MA or VA or Flamy Arrow DoT
			else if (sscanf(line, "(debuff (class %d)(skill_id %d)(buff_id %d)(base_damage %d)(damageC %d)(str %d)(agi %d)(damage_per_grade %d)(pvp_reduction %d)(duration %d)(duration_per_grade %d)(enabled %d))", &character, &skill_id, &buff_id, &base_damage, &damageC, &str, &agi, &damage_per_grade, &pvp_reduction, &duration, &duration_per_grade, &enabled) == 12)
			{
				...
				IConfig::DebuffCalc[{character, skill_id}].enabled = enabled;
			}
			//Heals
	...
	if (it == IConfig::dungeon_map.end())
	{
		...
		if (fileinstance != NULL)
		{
			...
			while (fgets(line, sizeof line, fileinstance) != NULL)
			{
				...
				{
					CDungeon::DungSummon summon(instance_id, wave_id, is_boss_wave, mini_boss_id, mini_boss_spawn_chance, x, y, monster_id, monster_amount);
					...
					{
						CDungeon::Spawn temp(x, y, monster_id, monster_amount);
						p->second.waves_map[wave_id].wave_vec.push_back(temp);
					}
				}
			}
			fclose(fileinstance);
